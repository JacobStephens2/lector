# kokoro - local TTS backend for sotto

A small Flask service that synthesizes speech with **Kokoro-82M** (ONNX) on CPU
and returns MP3. sotto calls it on localhost instead of a hosted TTS API, so
...
permissively licensed audio, so sotto can finally *name* its model's provenance
...
# voice previews for the sotto home page (commit the results)
.venv/bin/python generate_samples.py
```

Then install the service and point sotto at it:
...
the line) and restart sotto. Nothing else changes.
...
but an audiobook-length document (~1 hour of audio) takes ~40 min. sotto runs
